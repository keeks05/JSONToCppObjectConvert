...
Comment::Comment(std::string arg_profile_id, std::string arg_post_id, std::string arg_comment_id,
...
bool Comment::Jdump(Json::Value *input_json_ptr)
{
      ...
      (((*input_json_ptr)["reactions"]["data"]).isArray() == true))
  {
    ...
    for (j = 0; j < ((*input_json_ptr)["reactions"]["data"]).size(); j++)
    {
      ...
      // to be completed by ecs36b_f2021 students for hw6
      // ************************************************
      Json::Value l_jv = (*input_json_ptr)["reactions"]["data"][j];
